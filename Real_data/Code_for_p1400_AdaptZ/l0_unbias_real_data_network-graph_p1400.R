...
while(j<p){
  ...
  # X<-as.matrix(X)
  y<-as.vector(y)
  best_ebeta<-asdar(X,nX,dx,y,14,n,p) #14=floor(n/(logploglogn)),this time we use whole dataset
...
save(sdar_unbias_partial_correlation_star,file="L0_venn_plot.RData")
...
V(g1)$size=igraph::degree(g1)%>%+0.1%>%pow(0.2)*2
E(g1)$width=E(g1)$weight%>%+1%>%pow(1.7)
set.seed(1)
l=layout_with_graphopt(g1)
save(g1,l,file="L0_unbias_real_data_igraphplot_1400.RData")
plot(g1, vertex.label.color="cornflowerblue", 
     vertex.label.cex=0.3, vertex.label.dist=0.5,vertex.frame.color="azure4",layout=l)
...
sparsity_1
save(sdar_partial_correlation_threshold1,file="L0_venn_plot_threshold1.RData")
...
V(g1_t1)$size=igraph::degree(g1_t1)%>%+0.1%>%pow(0.2)*2
E(g1_t1)$width=E(g1_t1)$weight%>%+1%>%pow(1.7)
