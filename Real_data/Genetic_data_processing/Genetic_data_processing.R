...
#Select 300 differentially expressed genes based on BH
...
#Select 1400 differentially expressed genes based on AdaptZ
TransZvalue <- {}
for(i in 1:nrow(nrDEG1)){
  if(nrDEG1$t[i]>0){
    TransZvalue[i] <- qnorm(1-nrDEG1$P.Value[i]/2)
  }
  else{TransZvalue[i] <- -qnorm(1-nrDEG1$P.Value[i]/2)}
}
nrDEG1$TransZvalue <- TransZvalue
nrDEG1
...
alpha <- 0.05
adaptZ <- adaptZ.func(TransZvalue, alpha)
nrDEG1$lfdr <- adaptZ$lfdr
sum(nrDEG1$lfdr<adaptZ$threshold)
which(nrDEG1$lfdr<adaptZ$threshold)

Test_log2_genename<-rownames(nrDEG1[which(nrDEG1$lfdr<adaptZ$threshold),])
Test_log2_genedata<-MDA_log2[Test_log2_genename,]
Test_log2_genedata<-as.data.frame(Test_log2_genedata)
PCR_Test_log2_genedata<-Test_log2_genedata[,PCR_name]
RD_Test_log2_genedata<-Test_log2_genedata[,RD_name]

write.csv(PCR_Test_log2_genedata,file = "Log2/PCR_Test_log2_genedata_lfdr1400.csv")
write.csv(RD_Test_log2_genedata,file = "Log2/RD_Test_log2_genedata_lfdr1400.csv")
